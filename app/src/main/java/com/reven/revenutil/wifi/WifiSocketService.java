...
public class WifiSocketService extends Service
{
        ...
        ThreadUtil.sNewCachedThreadPool.execute(() -> {
            try
            {
                socket = new Socket(ip, Integer.parseInt(port));
                 Log.d("miss","socket connect success");
    ...
    public void send(String data)
    {
        ...
        if (data.length() != 24)
        {
             Log.d("miss","数据错误：" + data);
            return;
        }
         Log.d("miss","send wifi data：" + data);
        ...
        ThreadUtil.sNewCachedThreadPool.execute(() -> {
            if (socketStatus)
            {
                ...
                {
                     Log.d("miss","send data IOException：" + data);
                    e.printStackTrace();
                    close();
                    mWifiSocketCallBack.onReceiveData("连接失败");
                }
            ...
            {
                mWifiSocketCallBack.onReceiveData("连接失败");
                 Log.d("miss","socketStatus false,cant send data：" + data);
            }
        });
    }
