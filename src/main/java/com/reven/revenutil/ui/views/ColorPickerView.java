...
public class ColorPickerView extends View
{
    ...
    public ColorPickerView(Context context, @Nullable AttributeSet attrs, int defStyleAttr)
    {
        ...
        final TypedArray array = context.getTheme()
                .obtainStyledAttributes(attrs, R.styleable.ColorPickerView, defStyleAttr, 0);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec)
    {
        ...
        width = Math.max(width, orientation ==
                                Orientation.HORIZONTAL ? defaultSizeLong : defaultSizeShort);
        height = Math.max(height, orientation ==
                                  Orientation.HORIZONTAL ? defaultSizeShort : defaultSizeLong);
    ...
    private void calculBounds()
    {
        ...
        {
            if (w <= h)
            { // HORIZONTAL 模式，然而宽却小于高，以 6 ：1 的方式重新计算高
                size = w / 6;
            }
        }
        else
        ...
        int t, l, b, r;
        final int s = each * 5 / 2;//为了缩小圆球，将色条宽度由3设置为5，这样，色条宽了，圆球小了

        if (orientation == Orientation.HORIZONTAL)
        {
            l = mLeft + mRadius;//不知为何加了padding
            r = mRight - mRadius;
    ...
    private void createIndicatorBitmap()
    {
        ...
        int radius = 3;
        paintForIndicator.setShadowLayer(radius, 0, 0, Color.WHITE);
     ...
     * 是否发送了越界指令
     */
    private boolean mHasSendBound = false;
    ...
    public boolean onTouchEvent(MotionEvent event)
    {
        ...
        mPosition = (int) ((pointX / everyWidth));
        if (mPosition == 0)
        {
            mPosition = 1;
        }
        ...
        {
            if (colorPickerChangeListener != null && !mHasSendBound)
            {
